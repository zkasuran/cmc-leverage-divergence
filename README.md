...
the loss from -33.6% to -12.6%** on this short down-only window (`npm run cmc20`).
Both probabilistic Sharpes sit below 0.5 (0.16 overlay vs 0.18 buy-and-hold), so
...
- **Trend gate.** Below the 100-day trend the allocation is cut to 20% (risk-off),
  ...
  the CMC20 drawdown cut comes from, 41.9% to 15.3% over a down-only sample.
...
|--|------:|-------------:|-------------:|
| CMC20 buy-and-hold | -33.6% | 41.9% | 0.18 |
| **Funding-regime overlay** | **-12.6%** | **15.3%** | **0.16** |
...
**below 0.5 (0.16 overlay vs 0.18 buy-and-hold)**, so neither shows a
